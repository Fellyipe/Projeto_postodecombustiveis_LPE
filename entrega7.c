...
int main(int argc, char** argv){
    ...
    struct TCarro *CarrosF, *CarrosA;
    struct TVenda *Vendas,*Vendas2;
    
    CarrosA = (struct TCarro *) calloc(5,sizeof(struct TCarro));
    Vendas = (struct TVenda *) calloc(5,sizeof(struct TVenda));
    if(CarrosA==NULL || Vendas==NULL){
	...
	preco = definirPreco();
    maxCar = definirFila();
    CarrosF = (struct TCarro *) calloc(maxCar,sizeof(struct TCarro));
            ...
            case 2:
                ...
                if(valido==1){
                    if(lit<=litRest){
                        ...
                        litVend += lit;
                        vendGas += preco*lit;
                        Vendas = (struct TVenda *) realloc(Vendas,(carAten+1) * sizeof(struct TVenda));
                        Vendas[carAten].litros = lit;
                        Vendas[carAten].total = preco*lit;
                        Vendas[carAten].preco = preco;
                        CarrosA = (struct TCarro *) realloc(CarrosA,(carAten+1) * sizeof(struct TCarro));
                    ...
                    switch(operRA){
                        ...
                        case 9:
                            ...
                            for(int i=0;i<carAten;i++){
                                printf("\n%d - %.2f litros × %.2fR$ por litro = total %.2fR$",i+1,Vendas[i].litros,Vendas[i].preco,Vendas[i].total);
                            }
                            limpaBuffer();
                            enter();
                            break;
                        case 10:
                            ...
                            for(int i=0;i<carAten;i++){
                                printf("\n%d - %s cor %s, ano de %d",i+1,CarrosA[i].modelo,CarrosA[i].cor,CarrosA[i].ano);
                            }
                            limpaBuffer();
                            enter();
                            break;
                    ...
                    switch(operRA){
                        ...
                        case 4:
                            if(carFila!=0){
                                ...
                                maxCar = definirFila();
                                CarrosF = (struct TCarro *) realloc(CarrosF,(maxCar) * sizeof(struct TCarro));
                            }
                            break;
                        case 5:
